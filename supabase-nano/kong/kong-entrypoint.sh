...
# JWT_SECRET. Miget generates JWT_SECRET (random) but cannot produce a signed
# JWT, so the two API keys are derived here. Fixed claims => deterministic =>
# every supabase service that derives the same way gets identical keys.
if [ -n "$JWT_SECRET" ] && [ -z "$ANON_KEY" ]; then
  _b64url() { base64 | tr '+/' '-_' | tr -d '=\n'; }
  _hdr=$(printf '%s' '{"alg":"HS256","typ":"JWT"}' | _b64url)
  for _role in anon service_role; do
    _pay=$(printf '%s' "{\"role\":\"$_role\",\"iss\":\"supabase\",\"iat\":1700000000,\"exp\":2000000000}" | _b64url)
    _sig=$(printf '%s' "$_hdr.$_pay" | openssl dgst -binary -sha256 -hmac "$JWT_SECRET" | _b64url)
    if [ "$_role" = "anon" ]; then export ANON_KEY="$_hdr.$_pay.$_sig"; else export SERVICE_ROLE_KEY="$_hdr.$_pay.$_sig"; fi
  done
